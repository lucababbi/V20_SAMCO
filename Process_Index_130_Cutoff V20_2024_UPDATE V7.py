    ...
    if pd.to_datetime(date).strftime("%Y-%m-%d") == pd.to_datetime(Dates_Frame.head(1)["Cutoff"].values[0]).strftime("%Y-%m-%d"):
        ...
        if date <= pd.to_datetime("2023-02-28"):
            ...
            temp_Input = temp_Input[
                ~(
                    (temp_Input["Country"] == "CN") &
                    (temp_Input["Instrument_Name"].str.contains("'A'") | temp_Input["Instrument_Name"].str.contains("(CCS)")) &
                    (temp_Input["Index_Symbol"] == "SWESCGV") &
                    (temp_Input["Currency"] == "CNY")
                )
            ]
        else:
        ...
        for country in temp_Input["Country"].unique():
            ...
            if country == "KR":
                ...
                temp = pd.concat([temp[temp["Rank"] < Threshold_China], temp[temp["Rank"] >= Threshold_China].iloc[0:1]])
            else:
        ...
        if date == pd.to_datetime("2021-02-26"):
            print("ALT")
...
Output[[
    ...
    "FOR_FF"
]].to_csv(r"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V20_SAMCO\Output\V7_Base\Final_Buffer_V" + str(Version) + "_2024.csv")
...
FOR_Removed[[
    ...
    "FOR_FF",
    "Weight"
]].to_csv(r"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V20_SAMCO\Output\V7_Base\Final_Buffer_V" + str(Version) + "_FOR_Removed_Securities_2024.csv")